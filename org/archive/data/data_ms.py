...
def check_irgan_data(dataset=''):
	#fold_num = 1
	#dir_data, num_features, has_comment, query_level_scale, multi_level_rele, _ = get_data_meta(dataset='', pc='imac')

	file_train = '/Users/dryuhaitao/Downloads/irgan_used_data/train.txt'
	file_test = '/Users/dryuhaitao/Downloads/irgan_used_data/test.txt'

	original_train_Qs, _ = load_ms_data_qm(in_file=file_train, has_comment=True, query_level_scale=False)
	#original_vali_Qs, _ = load_ms_data_qm(in_file=file_vali, has_comment=has_comment, query_level_scale=query_level_scale)
	original_test_Qs, _ = load_ms_data_qm(in_file=file_test, has_comment=True, query_level_scale=False)

	num_queries = len(original_train_Qs) + len(original_test_Qs)
	print('Total queries:\t', num_queries, len(original_train_Qs), len(original_test_Qs))
	#num_docs = get_doc_num(original_train_Qs) + get_doc_num(original_vali_Qs) + get_doc_num(original_test_Qs)
	'''
	print('Total docs:\t', num_docs)

	min_doc, max_doc, sum_rele = get_min_max_docs(train_Qs=original_train_Qs, vali_Qs=original_vali_Qs,
												  test_Qs=original_test_Qs)
	print('min, max documents per query', min_doc, max_doc, sum_rele)
	print('avg rele documents per query', sum_rele * 1.0 / num_queries)
	print('avg documents per query', num_docs * 1.0 / num_queries)
	'''

def check_irgan_data_2(dataset=''):
	#fold_num = 1
	#dir_data, num_features, has_comment, query_level_scale, multi_level_rele, _ = get_data_meta(dataset='', pc='imac')

	#file_train = '/Users/dryuhaitao/Downloads/irgan_used_data/train.txt'
	#file_test = '/Users/dryuhaitao/Downloads/irgan_used_data/test.txt'
	file_test = '/Users/dryuhaitao/WorkBench/Corpus/LETOR4.0/MQ2008-semi/Fold1/test.txt'
	...
	#num_queries = len(original_train_Qs) + len(original_test_Qs)
	print('test queries:\t', len(original_test_Qs))
	#num_docs = get_doc_num(original_train_Qs) + get_doc_num(original_vali_Qs) + get_doc_num(original_test_Qs)
	'''
	print('Total docs:\t', num_docs)
	...
	for test_Q in original_test_Qs:
		print(test_Q.qid, len(test_Q.std_label_vec))

		#print(test_Q.feature_mat)

		print(test_Q.std_label_vec[-10:-1])
...
if __name__ == '__main__':
	...
	#check_irgan_data() #Total queries:	 784 628 156
