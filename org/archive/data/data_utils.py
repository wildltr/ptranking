...
def get_data_loader(original_file=None, has_comment=None, query_level_scale=None,
					min_docs=None, min_rele=None, need_pre_sampling=None, sample_times_per_q=None, shuffle = False, batch_size = 1):
	source_buffer = data_ms.get_qm_file_buffer(in_file=original_file, has_comment=has_comment, query_level_scale=query_level_scale)
	...
	if os.path.exists(tor_file_buffer):
		...
		original_Qs, source_buffer = data_ms.load_ms_data_qm(in_file=original_file, has_comment=has_comment, query_level_scale=query_level_scale)
