...
# Created by Hai-Tao Yu | 18/10/30 | https://y-research.github.io
...
from org.archive.l2r_global import global_gpu as gpu, global_device as device, torch_zero, torch_one, cpu_torch_one, cpu_torch_zero
...
class Pair_IR_GAN(AdversarialMachine):
    ''''''
...
def get_pair_irgan_paras_str(model_para_dict, log=False):
    s1 = ':' if log else '_'

    d_epoches, g_epoches, temperature, ad_training_order, loss_type = model_para_dict['d_epoches'],\
          model_para_dict['g_epoches'],  model_para_dict['temperature'], model_para_dict['ad_training_order'],\
                                                                      model_para_dict['loss_type']

    pair_irgan_paras_str = s1.join([str(d_epoches), str(g_epoches), '{:,g}'.format(temperature), ad_training_order, loss_type])
