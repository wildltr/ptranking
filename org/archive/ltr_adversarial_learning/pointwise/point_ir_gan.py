...
class Point_IR_GAN(AdversarialMachine):
    ''''''
...
def get_point_irgan_paras_str(model_para_dict, log=False):
    s1 = ':' if log else '_'
    ...
    point_irgan_paras_str = s1.join([str(d_epoches), str(g_epoches), '{:,g}'.format(temperature), ad_training_order])
